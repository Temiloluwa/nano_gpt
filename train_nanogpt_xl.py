...
if __name__ == "__main__":
    train_data = get_train_data(tokenizer, processed_file_path)
    gpt_model = FullBigramLanguageAttentionModel(vocab_size, 
                                                embedding_dims, 
                                                n_heads,
                                                dropout,
                                                n_blocks).to(device)
    optimizer = torch.optim.Adam(gpt_model.parameters(), lr=lr)
    gpt_model = train_model(gpt_model, optimizer, train_data, batchsize, context)
    current_datetime = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    save_path = f"models/model_{current_datetime}_batch{batchsize}_context{context}_heads{n_heads}_embed{embedding_dims}_lr{lr}_blocks{n_blocks}_epochs{str(epochs)}.pth"
    save_model(gpt_model, save_path)
    print("Generated save path:", save_path)
